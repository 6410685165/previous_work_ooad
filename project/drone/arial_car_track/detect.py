...
from arial_car_track.utils.torch_utils import select_device, load_classifier, time_synchronized, TracedModel
...
def run_detect(opt,version='yolov7.pt'):
    opt = opt
    print(opt)
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in [version]:
                detect(opt).detect(opt)
