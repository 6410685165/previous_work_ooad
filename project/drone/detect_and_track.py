...
def check_enter_exit_loop(track):
    ...
    for loop in loops:
        ...
        if len(track.centroidarr)>20:
            ...
            #check entering line
            line_enter_check_and_set(loop,track,tp1,tp2,pt0,pt1)
            
            #check exit line left straight and right
            line_exit_check_and_set(loop,track,tp1,tp2,pt1,pt2,"left")
            line_exit_check_and_set(loop,track,tp1,tp2,pt2,pt3,"straight")
...
def mymain(cmd = False, custom_arg=None):
    ...
    parser.add_argument('--project', default='media/result', help='save results to project/name')
    ...
    if cmd==True:
        ...
        opt = parser.parse_args(custom_arg)
